...
- **覆盖面**:本期**完整 token 化**仅编辑器五面板(工具栏 / 素材库 / 画布 / 属性 / 时间轴)。
  - 范围澄清(2026-06-22,经 Codex 审查后确认):`data-ui="v2"` 挂在 `<html>` 全局,故 v2 的**基础变量重映射 + Inter 字体**会惠及登录页 / 项目列表页 / 弹窗页。这是**有意保留**——让整个应用共享精炼基础、更协调(实测登录页 v2 视觉良性,无破坏)。
  - 这些页面有**自有的硬编码字面量**(如 `ProjectsPage.css` 等)尚未 token 化,故当前为「精炼基础 + 自有旧色」的混合态。把它们也完整刷新(token 化各自字面量)= **后续期**,不在本期。
